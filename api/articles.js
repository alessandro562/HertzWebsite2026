...
export default async function handler(req, res) {
  ...
  if (featured === 'true') {
    result = result.filter(a => a.engagement.featured);
  }
  ...
  const articles = result.map(({ slug, title, subtitle, excerpt, metadata, content, engagement }) => ({
    slug, title, subtitle, excerpt, metadata,
    content: { readingTimeMinutes: content.readingTimeMinutes },
    engagement
  }));

  return res.status(200).json({ ok: true, articles, total: articles.length });
}
